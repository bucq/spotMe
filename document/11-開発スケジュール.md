...
> [[10-監視と運用]] | [[spotMe設計書]] | Next: [[12-コストとスケーリング]]
...
- [x] **Day 1-2**: GCP プロジェクト作成・IAM設定
  ```bash
  # GCP プロジェクト作成
  gcloud projects create spotMe-mvp
  gcloud config set project spotMe-mvp
...
- [x] **Day 3-4**: Cloud SQL（PostgreSQL）セットアップ
  ```bash
  # Cloud SQL インスタンス作成
  gcloud sql instances create spotMe-db \
...
- [x] **Day 1-3**: React Native 開発環境構築
  ...
  # プロジェクト作成
  npx react-native init spotMeApp --template react-native-template-typescript
  ```
...
- [x] **Day 4-5**: Cloud Build デプロイメント
  ...
  steps:
    - name: 'gcr.io/cloud-builders/docker'
      args: ['build', '-t', 'gcr.io/$PROJECT_ID/spotMe-api', '.']

    - name: 'gcr.io/cloud-builders/gcloud'
      args: ['run', 'deploy', 'spotMe-api', '--image', 'gcr.io/$PROJECT_ID/spotMe-api']
  ```
...
- [x] **Day 1-2**: TestFlight / Play Console ベータ配信
  ```bash
  # iOS ベータ配信
  cd ios && xcodebuild -workspace spotMeApp.xcworkspace -scheme spotMeApp archive
...
- [x] **Day 5-6**: 本番環境構築・設定
  ```bash
  # 本番環境デプロイ
  gcloud run deploy spotMe-api-prod \
    --image gcr.io/spotMe-prod/spotMe-api:latest \
    --region asia-northeast1 \
    --min-instances 1 \
    --max-instances 100
  ```
