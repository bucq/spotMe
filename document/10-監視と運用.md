# 10 - 監視と運用

> [[09-セキュリティとプライバシー]] | [[spotMe設計書]] | Next: [[11-開発スケジュール]]

## 📊 監視戦略

### 監視の4つの柱

```
🔍 メトリクス     📋 ログ
├─ システム      ├─ アプリケーション
├─ アプリ        ├─ セキュリティ
├─ ビジネス      ├─ 監査
└─ ユーザー      └─ エラー

📈 トレース      🚨 アラート
├─ 分散追跡      ├─ しきい値
├─ 依存関係      ├─ 異常検知
├─ ボトルネック  ├─ 通知
└─ パフォーマンス └─ エスカレーション
```

---

## 📈 メトリクス監視

### システムメトリクス

#### Cloud Run メトリクス

| メトリクス | しきい値 | アラート | 対応 |
|------------|----------|----------|------|
| **CPU使用率** | > 80% | Warning | オートスケール確認 |
| **メモリ使用率** | > 85% | Warning | メモリ制限見直し |
| **リクエスト数/秒** | > 1000 | Info | スケール状況確認 |
| **レスポンス時間 P95** | > 1秒 | Critical | パフォーマンス調査 |
| **エラー率** | > 1% | Critical | 緊急対応 |
| **起動時間** | > 10秒 | Warning | コールドスタート対策 |

#### Cloud SQL メトリクス

| メトリクス | しきい値 | アラート | 対応 |
|------------|----------|----------|------|
| **接続数** | > 80% | Warning | 接続プール調整 |
| **CPU使用率** | > 80% | Warning | インスタンス拡張 |
| **ディスク使用率** | > 90% | Critical | ストレージ拡張 |
| **レプリケーション遅延** | > 5秒 | Warning | レプリカ確認 |
| **スロークエリ** | > 1秒 | Warning | クエリ最適化 |

### アプリケーションメトリクス

#### ビジネスメトリクス

```python
from prometheus_client import Counter, Histogram, Gauge
import time

# カスタムメトリクス定義
status_updates_total = Counter(
    'spotMe_status_updates_total',
    'Total number of status updates',
    ['status_type', 'user_id']
)

status_update_duration = Histogram(
    'spotMe_status_update_duration_seconds',
    'Time spent processing status updates'
)

active_users = Gauge(
    'spotMe_active_users',
    'Number of currently active users'
)

firestore_sync_errors = Counter(
    'spotMe_firestore_sync_errors_total',
    'Total number of Firestore sync errors'
)

class MetricsCollector:
    """アプリケーションメトリクス収集"""

    @staticmethod
    def record_status_update(status_type: str, user_id: str, duration: float):
        """ステータス更新メトリクス"""
        status_updates_total.labels(
            status_type=status_type,
            user_id=user_id
        ).inc()

        status_update_duration.observe(duration)

    @staticmethod
    def update_active_users(count: int):
        """アクティブユーザー数更新"""
        active_users.set(count)

    @staticmethod
    def record_sync_error(error_type: str):
        """同期エラー記録"""
        firestore_sync_errors.labels(error_type=error_type).inc()

# 使用例
async def update_user_status(event_id: str, user_id: str, status_data: dict):
    start_time = time.time()

    try:
        # ステータス更新処理
        result = await process_status_update(event_id, user_id, status_data)

        # メトリクス記録
        duration = time.time() - start_time
        MetricsCollector.record_status_update(
            status_data['status_type'],
            user_id,
            duration
        )

        return result

    except FirestoreSyncError as e:
        MetricsCollector.record_sync_error("firestore_sync")
        raise
```

### ユーザーエクスペリエンスメトリクス

#### React Native パフォーマンス

```typescript
import analytics from '@react-native-firebase/analytics';
import perf from '@react-native-firebase/perf';

class UXMetrics {
  /**
   * 画面読み込み時間測定
   */
  static async measureScreenLoad(screenName: string) {
    const trace = perf().newTrace(`screen_load_${screenName}`);
    await trace.start();

    // 画面読み込み完了時
    await trace.stop();
  }

  /**
   * ステータス更新レスポンス時間
   */
  static async measureStatusUpdate(statusType: string) {
    const startTime = Date.now();

    try {
      await updateStatus(statusType);

      const duration = Date.now() - startTime;

      // Firebase Analytics に記録
      await analytics().logEvent('status_update_performance', {
        status_type: statusType,
        duration_ms: duration,
        success: true
      });

    } catch (error) {
      const duration = Date.now() - startTime;

      await analytics().logEvent('status_update_performance', {
        status_type: statusType,
        duration_ms: duration,
        success: false,
        error_type: error.type
      });
    }
  }

  /**
   * アプリクラッシュ追跡
   */
  static reportCrash(error: Error, screen: string) {
    analytics().logEvent('app_crash', {
      error_message: error.message,
      screen_name: screen,
      timestamp: Date.now()
    });
  }
}
```

---

## 📋 ログ管理

### 構造化ログ

#### ログレベル定義

| レベル | 用途 | 例 | 保持期間 |
|--------|------|----|---------|
| **DEBUG** | 開発時詳細情報 | 関数呼び出し、変数値 | 7日 |
| **INFO** | 一般的な情報 | API呼び出し、ステータス変更 | 30日 |
| **WARNING** | 注意が必要な状況 | 遅延、リトライ | 90日 |
| **ERROR** | エラー発生 | 例外、失敗 | 1年 |
| **CRITICAL** | システム停止レベル | サービス停止、データ破損 | 永続 |

#### ログ形式標準化

```python
import logging
import json
from datetime import datetime
from typing import Optional

class StructuredLogger:
    """構造化ログ出力"""

    def __init__(self, service_name: str):
        self.service_name = service_name
        self.logger = logging.getLogger(service_name)

    def log(
        self,
        level: str,
        message: str,
        user_id: Optional[str] = None,
        event_id: Optional[str] = None,
        trace_id: Optional[str] = None,
        **kwargs
    ):
        """構造化ログ出力"""
        log_entry = {
            "timestamp": datetime.utcnow().isoformat(),
            "severity": level.upper(),
            "service": self.service_name,
            "message": message,
            "trace_id": trace_id,
            "user_id": user_id,
            "event_id": event_id,
            **kwargs
        }

        # JSON形式で出力（Cloud Logging向け）
        self.logger.log(
            getattr(logging, level.upper()),
            json.dumps(log_entry, ensure_ascii=False)
        )

    def info_status_update(
        self,
        user_id: str,
        event_id: str,
        old_status: str,
        new_status: str,
        trace_id: str
    ):
        """ステータス更新ログ"""
        self.log(
            "info",
            "User status updated",
            user_id=user_id,
            event_id=event_id,
            trace_id=trace_id,
            old_status=old_status,
            new_status=new_status,
            operation="status_update"
        )

    def error_firestore_sync(
        self,
        user_id: str,
        event_id: str,
        error: Exception,
        trace_id: str
    ):
        """Firestore同期エラーログ"""
        self.log(
            "error",
            "Firestore sync failed",
            user_id=user_id,
            event_id=event_id,
            trace_id=trace_id,
            error_type=type(error).__name__,
            error_message=str(error),
            operation="firestore_sync"
        )

# 使用例
logger = StructuredLogger("spotMe-api")

async def update_user_status(event_id: str, user_id: str, status_data: dict):
    trace_id = generate_trace_id()

    try:
        old_status = await get_current_status(event_id, user_id)
        new_status = await save_status(event_id, user_id, status_data)

        # 成功ログ
        logger.info_status_update(
            user_id, event_id,
            old_status.status_type if old_status else None,
            new_status.status_type,
            trace_id
        )

        return new_status

    except Exception as e:
        # エラーログ
        logger.error_firestore_sync(user_id, event_id, e, trace_id)
        raise
```

### ログ検索・分析

#### BigQuery ログ分析

```sql
-- ステータス更新パフォーマンス分析
SELECT
  JSON_EXTRACT_SCALAR(jsonPayload, '$.user_id') as user_id,
  JSON_EXTRACT_SCALAR(jsonPayload, '$.old_status') as old_status,
  JSON_EXTRACT_SCALAR(jsonPayload, '$.new_status') as new_status,
  TIMESTAMP_DIFF(
    timestamp,
    LAG(timestamp) OVER (
      PARTITION BY JSON_EXTRACT_SCALAR(jsonPayload, '$.user_id')
      ORDER BY timestamp
    ),
    MILLISECOND
  ) as update_interval_ms
FROM `spotMe.logs.cloudrun`
WHERE
  DATE(timestamp) = CURRENT_DATE()
  AND JSON_EXTRACT_SCALAR(jsonPayload, '$.operation') = 'status_update'
  AND severity = 'INFO'
ORDER BY timestamp DESC;

-- エラー率分析
SELECT
  DATE(timestamp) as date,
  COUNT(*) as total_requests,
  COUNTIF(severity = 'ERROR') as error_count,
  ROUND(COUNTIF(severity = 'ERROR') / COUNT(*) * 100, 2) as error_rate_percent
FROM `spotMe.logs.cloudrun`
WHERE
  DATE(timestamp) >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)
GROUP BY date
ORDER BY date DESC;

-- ユーザー行動分析
SELECT
  JSON_EXTRACT_SCALAR(jsonPayload, '$.new_status') as status_type,
  COUNT(*) as update_count,
  COUNT(DISTINCT JSON_EXTRACT_SCALAR(jsonPayload, '$.user_id')) as unique_users,
  ROUND(AVG(TIMESTAMP_DIFF(
    LEAD(timestamp) OVER (
      PARTITION BY JSON_EXTRACT_SCALAR(jsonPayload, '$.user_id')
      ORDER BY timestamp
    ),
    timestamp,
    SECOND
  )), 2) as avg_duration_seconds
FROM `spotMe.logs.cloudrun`
WHERE
  DATE(timestamp) = CURRENT_DATE()
  AND JSON_EXTRACT_SCALAR(jsonPayload, '$.operation') = 'status_update'
GROUP BY status_type
ORDER BY update_count DESC;
```

---

## 🚨 アラートシステム

### アラート階層

#### レベル別対応

| レベル | 通知先 | 対応時間 | エスカレーション |
|--------|--------|----------|------------------|
| **Info** | Slack | - | なし |
| **Warning** | Slack + Email | 30分 | 1時間後にエスカレート |
| **Critical** | Slack + Email + SMS | 5分 | 15分後にエスカレート |
| **Emergency** | 全チャネル + 電話 | 即座 | 5分後にマネージャー |

#### アラートルール

```yaml
# Cloud Monitoring アラートポリシー
api_error_rate:
  condition:
    metric: "compute.googleapis.com/instance/cpu/utilization"
    filter: 'resource.type="gce_instance"'
    threshold: 0.8
    duration: 300s
  notification:
    - slack-channel: "#alerts"
    - email: "dev-team@spotMe.com"

status_update_latency:
  condition:
    metric: "custom.googleapis.com/status_update_duration"
    aggregation: "95th percentile"
    threshold: 2.0
    duration: 180s
  notification:
    - slack-channel: "#performance"

firestore_sync_errors:
  condition:
    metric: "custom.googleapis.com/firestore_sync_errors"
    rate: "> 10/minute"
    duration: 60s
  severity: "critical"
  notification:
    - slack-channel: "#alerts"
    - email: "oncall@spotMe.com"
    - sms: "+81-80-1234-5678"
```

### 異常検知

#### 機械学習ベース検知

```python
from sklearn.ensemble import IsolationForest
import numpy as np

class AnomalyDetector:
    """異常検知システム"""

    def __init__(self):
        self.model = IsolationForest(contamination=0.1)
        self.is_trained = False

    def train(self, normal_data: np.ndarray):
        """正常データでモデル訓練"""
        self.model.fit(normal_data)
        self.is_trained = True

    def detect_anomaly(self, metrics: dict) -> bool:
        """異常検知"""
        if not self.is_trained:
            return False

        # メトリクスを特徴量ベクトルに変換
        features = np.array([
            metrics['response_time'],
            metrics['error_rate'],
            metrics['request_count'],
            metrics['cpu_usage'],
            metrics['memory_usage']
        ]).reshape(1, -1)

        # 異常スコア計算
        anomaly_score = self.model.decision_function(features)[0]

        # しきい値による判定
        return anomaly_score < -0.5

# 使用例
anomaly_detector = AnomalyDetector()

async def check_system_health():
    """システムヘルスチェック"""
    current_metrics = {
        'response_time': await get_avg_response_time(),
        'error_rate': await get_error_rate(),
        'request_count': await get_request_count(),
        'cpu_usage': await get_cpu_usage(),
        'memory_usage': await get_memory_usage()
    }

    if anomaly_detector.detect_anomaly(current_metrics):
        await send_anomaly_alert(current_metrics)
```

---

## 🛠️ 運用プロセス

### インシデント対応

#### インシデント分類

| 重要度 | 定義 | 目標対応時間 | 例 |
|--------|------|-------------|-----|
| **P0** | サービス全停止 | 15分 | API完全停止、データ消失 |
| **P1** | 主要機能停止 | 1時間 | ステータス更新不可 |
| **P2** | 一部機能影響 | 4時間 | 位置情報表示異常 |
| **P3** | 軽微な問題 | 24時間 | UI表示崩れ |

#### 対応フロー

```
🔍 検知
├─ 自動監視システム
├─ ユーザー報告
└─ 定期ヘルスチェック
    │
    ↓
🚨 トリアージ
├─ 重要度判定
├─ 影響範囲確認
└─ 担当者アサイン
    │
    ↓
🔧 対応
├─ 応急処置
├─ 根本原因調査
└─ 修正実装
    │
    ↓
✅ 解決
├─ 機能回復確認
├─ ユーザー通知
└─ ポストモーテム作成
```

#### インシデント管理

```python
from enum import Enum
from dataclasses import dataclass
from datetime import datetime
from typing import List, Optional

class IncidentSeverity(Enum):
    P0 = "critical"
    P1 = "high"
    P2 = "medium"
    P3 = "low"

class IncidentStatus(Enum):
    OPEN = "open"
    INVESTIGATING = "investigating"
    IDENTIFIED = "identified"
    MONITORING = "monitoring"
    RESOLVED = "resolved"

@dataclass
class Incident:
    id: str
    title: str
    description: str
    severity: IncidentSeverity
    status: IncidentStatus
    created_at: datetime
    updated_at: datetime
    assigned_to: Optional[str] = None
    tags: List[str] = None
    affected_users: int = 0

class IncidentManager:
    """インシデント管理"""

    @staticmethod
    async def create_incident(
        title: str,
        description: str,
        severity: IncidentSeverity,
        tags: List[str] = None
    ) -> Incident:
        """インシデント作成"""
        incident = Incident(
            id=generate_incident_id(),
            title=title,
            description=description,
            severity=severity,
            status=IncidentStatus.OPEN,
            created_at=datetime.utcnow(),
            updated_at=datetime.utcnow(),
            tags=tags or []
        )

        await save_incident(incident)

        # 重要度に応じた通知
        if severity in [IncidentSeverity.P0, IncidentSeverity.P1]:
            await send_critical_alert(incident)

        return incident

    @staticmethod
    async def update_incident_status(
        incident_id: str,
        new_status: IncidentStatus,
        update_message: str
    ):
        """インシデント状況更新"""
        incident = await get_incident(incident_id)
        incident.status = new_status
        incident.updated_at = datetime.utcnow()

        await save_incident(incident)
        await post_status_update(incident, update_message)

        # 解決時の処理
        if new_status == IncidentStatus.RESOLVED:
            await schedule_postmortem(incident)
```

### デプロイメント

#### Blue-Green デプロイメント

```yaml
# Cloud Run Blue-Green デプロイ
steps:
  # 1. Green環境にデプロイ
  - name: 'gcr.io/cloud-builders/gcloud'
    args:
      - 'run'
      - 'deploy'
      - 'spotMe-api-green'
      - '--image'
      - 'gcr.io/$PROJECT_ID/spotMe-api:$COMMIT_SHA'
      - '--region'
      - 'asia-northeast1'
      - '--no-traffic'

  # 2. ヘルスチェック
  - name: 'gcr.io/cloud-builders/curl'
    args:
      - '-f'
      - 'https://spotMe-api-green-xyz.a.run.app/health'

  # 3. スモークテスト
  - name: 'gcr.io/cloud-builders/gcloud'
    entrypoint: 'bash'
    args:
      - '-c'
      - |
        # API動作確認
        response=$(curl -s https://spotMe-api-green-xyz.a.run.app/api/v1/health)
        if [[ "$response" != *"healthy"* ]]; then
          echo "Health check failed"
          exit 1
        fi

  # 4. トラフィック切り替え（段階的）
  - name: 'gcr.io/cloud-builders/gcloud'
    args:
      - 'run'
      - 'services'
      - 'update-traffic'
      - 'spotMe-api'
      - '--to-revisions'
      - 'spotMe-api-green=10'
      - '--region'
      - 'asia-northeast1'

  # 5. 監視期間（5分）
  - name: 'gcr.io/cloud-builders/gcloud'
    entrypoint: 'bash'
    args:
      - '-c'
      - |
        sleep 300
        # エラー率チェック
        error_rate=$(gcloud monitoring metrics list --filter="metric.type=custom.googleapis.com/error_rate" --format="value(points[0].value.doubleValue)")
        if (( $(echo "$error_rate > 0.01" | bc -l) )); then
          echo "Error rate too high: $error_rate"
          # ロールバック
          gcloud run services update-traffic spotMe-api --to-revisions spotMe-api-blue=100
          exit 1
        fi

  # 6. 完全切り替え
  - name: 'gcr.io/cloud-builders/gcloud'
    args:
      - 'run'
      - 'services'
      - 'update-traffic'
      - 'spotMe-api'
      - '--to-revisions'
      - 'spotMe-api-green=100'
```

### バックアップ・復旧

#### 自動バックアップ

```bash
#!/bin/bash
# daily_backup.sh

set -e

# 変数設定
PROJECT_ID="spotMe-prod"
DB_INSTANCE="spotMe-db"
BUCKET="spotMe-backups"
DATE=$(date +%Y%m%d_%H%M%S)

echo "Starting backup at $(date)"

# 1. Cloud SQL バックアップ
echo "Creating Cloud SQL backup..."
gcloud sql backups create \
  --instance=$DB_INSTANCE \
  --description="Daily backup $DATE" \
  --project=$PROJECT_ID

# 2. Firestore エクスポート
echo "Exporting Firestore data..."
gcloud firestore export gs://$BUCKET/firestore/$DATE \
  --project=$PROJECT_ID

# 3. Cloud Storage バックアップ
echo "Backing up user data..."
gsutil -m cp -r gs://spotMe-user-images gs://$BUCKET/user-images/$DATE/

# 4. Secret Manager バックアップ
echo "Backing up secrets metadata..."
gcloud secrets list --format="value(name)" | while read secret; do
  gcloud secrets describe $secret --format=json > /tmp/${secret}.json
done
gsutil cp /tmp/*.json gs://$BUCKET/secrets/$DATE/

# 5. バックアップ検証
echo "Verifying backups..."
gsutil ls gs://$BUCKET/firestore/$DATE/ > /dev/null
gsutil ls gs://$BUCKET/user-images/$DATE/ > /dev/null

# 6. 古いバックアップ削除（30日以上）
echo "Cleaning up old backups..."
gsutil -m rm -r gs://$BUCKET/firestore/$(date -d '30 days ago' +%Y%m%d)_* || true
gsutil -m rm -r gs://$BUCKET/user-images/$(date -d '30 days ago' +%Y%m%d)_* || true

echo "Backup completed successfully at $(date)"

# 7. 結果通知
curl -X POST https://hooks.slack.com/services/... \
  -H 'Content-type: application/json' \
  --data "{\"text\":\"✅ Daily backup completed: $DATE\"}"
```

---

## 🔧 保守・運用手順

### 定期メンテナンス

#### 月次メンテナンス

```bash
#!/bin/bash
# monthly_maintenance.sh

echo "=== Monthly Maintenance $(date) ==="

# 1. 依存関係更新
echo "Updating dependencies..."
pip-review --auto
npm audit fix

# 2. セキュリティパッチ適用
echo "Applying security patches..."
gcloud sql patches apply --patch=... --instance=spotMe-db

# 3. データベース最適化
echo "Optimizing database..."
psql $DATABASE_URL << EOF
VACUUM ANALYZE;
REINDEX DATABASE spotMe;
EOF

# 4. 古いデータ削除
echo "Cleaning up old data..."
python scripts/cleanup_old_data.py

# 5. パフォーマンステスト
echo "Running performance tests..."
locust --headless -u 100 -r 10 -t 300s --host=https://api.spotMe.com

# 6. バックアップテスト
echo "Testing backup restoration..."
./scripts/test_backup_restore.sh

# 7. セキュリティスキャン
echo "Running security scan..."
nmap -sV api.spotMe.com
bandit -r app/

echo "=== Maintenance completed ==="
```

### キャパシティプランニング

#### 成長予測

```python
import pandas as pd
import numpy as np
from sklearn.linear_model import LinearRegression

class CapacityPlanner:
    """キャパシティ計画"""

    def __init__(self):
        self.user_growth_model = LinearRegression()
        self.load_prediction_model = LinearRegression()

    def analyze_growth_trend(self, historical_data: pd.DataFrame):
        """成長トレンド分析"""
        # ユーザー数の成長予測
        X = historical_data[['days_since_launch']].values
        y = historical_data['daily_active_users'].values

        self.user_growth_model.fit(X, y)

        # 3ヶ月後の予測
        future_days = np.array([[90]])  # 90日後
        predicted_users = self.user_growth_model.predict(future_days)[0]

        return {
            'predicted_dau_3months': int(predicted_users),
            'growth_rate_per_day': self.user_growth_model.coef_[0],
            'confidence_score': self.user_growth_model.score(X, y)
        }

    def recommend_scaling(self, current_metrics: dict, predicted_load: dict):
        """スケーリング推奨"""
        recommendations = []

        # Cloud Run 推奨
        if predicted_load['requests_per_second'] > 500:
            recommendations.append({
                'service': 'Cloud Run',
                'action': 'Increase max instances to 200',
                'reason': 'High request volume predicted'
            })

        # Cloud SQL 推奨
        if predicted_load['db_connections'] > current_metrics['max_connections'] * 0.8:
            recommendations.append({
                'service': 'Cloud SQL',
                'action': 'Upgrade to db-n1-standard-2',
                'reason': 'Connection pool will be exhausted'
            })

        return recommendations
```

---

**Next**: [[11-開発スケジュール]] - 12週間の詳細実装計画